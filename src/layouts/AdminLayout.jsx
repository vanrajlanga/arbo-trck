...
import { Menu, Home, Users, Map, Calendar, BarChart, Settings, LogOut, Wallet, Percent, Building2, MapPin, CreditCard, MessageSquare, Headphones, Shield, Server, ChevronDown, ChevronRight } from "lucide-react";
...
const AdminLayout = () => {
  ...
  return /*#__PURE__*/_jsxs("div", {
    ...
    children: [/*#__PURE__*/_jsx("div", {
      ...
      children: [/*#__PURE__*/_jsx("div", {
        className: "flex h-16 items-center justify-center px-4 bg-gray-800",
        ...
        className: "mt-5 px-2 h-[calc(100vh-5rem)] overflow-y-auto",
        children: [menuItems.map(item => /*#__PURE__*/_jsx("div", {
          className: "mb-1",
          children: item.subItems && item.subItems.length > 0 ? /*#__PURE__*/_jsxs(Collapsible, {
            open: expandedMenus.includes(item.id),
            onOpenChange: () => toggleMenu(item.id),
            children: [/*#__PURE__*/_jsxs(CollapsibleTrigger, {
              className: `w-full group flex items-center justify-between px-4 py-3 text-white hover:bg-white hover:bg-opacity-10 rounded-md transition-all ${isMenuActive(item) ? 'bg-white bg-opacity-10' : ''}`,
              ...
              }), expandedMenus.includes(item.id) ? /*#__PURE__*/_jsx(ChevronDown, {
                className: "h-4 w-4"
              ...
              className: "ml-4 mt-1",
              children: item.subItems.map(subItem => /*#__PURE__*/_jsx(Link, {
                to: subItem.path,
                className: `block px-4 py-2 text-sm text-gray-300 hover:bg-white hover:bg-opacity-10 rounded-md transition-all ${isSubItemActive(subItem.path) ? 'bg-white bg-opacity-10 text-white' : ''}`,
                children: subItem.title
              }, subItem.path))
            })]
          }) : /*#__PURE__*/_jsxs(Link, {
            to: item.path,
            className: `group flex items-center px-4 py-3 text-white hover:bg-white hover:bg-opacity-10 rounded-md transition-all ${isMenuActive(item) ? 'bg-white bg-opacity-10' : ''}`,
            children: [/*#__PURE__*/_jsx(item.icon, {
              className: "mr-3 h-5 w-5"
            }), /*#__PURE__*/_jsx("span", {
              children: item.title
        ...
        }, item.id)), /*#__PURE__*/_jsxs("button", {
          onClick: logout,
          className: "w-full mt-4 group flex items-center px-4 py-3 text-white hover:bg-white hover:bg-opacity-10 rounded-md transition-all",
