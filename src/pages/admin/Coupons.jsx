            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {coupons.length > 0 ? Math.round((activeCoupons.length / coupons.length) * 100) : 0}%
                </p>
              </div>
                ...
                {coupons.map((coupon) => (
                  <TableRow key={coupon.id}>
                    ...
                    <TableCell>
                      <Badge 
                        className={
                          !coupon.is_active ? "bg-gray-100 text-gray-800" :
                          isExpired(coupon.valid_until) ? "bg-red-100 text-red-800" :
                          "bg-green-100 text-green-800"
                        }
                      >
                        {!coupon.is_active ? "Inactive" : 
                         isExpired(coupon.valid_until) ? "Expired" : "Active"}
                      </Badge>
                    </TableCell>
                    <TableCell>
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Add a new coupon code for customers to use</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
              ...
              <div>
                <Label htmlFor="edit_discount_value">
                  Discount Value * {selectedCoupon.discount_type === 'percentage' ? '(%)' : '(₹)'}
                </Label>
                <Input
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setSelectedCoupon({...selectedCoupon, min_order_amount: Number(e.target.value)})}
                />
              </div>

              <div>
                <Label htmlFor="edit_max_discount_amount">Maximum Discount Amount (₹)</Label>
                <Input
                  id="edit_max_discount_amount"
                  type="number"
                  min="0"
                  value={selectedCoupon.max_discount_amount || 0}
                  onChange={(e) => setSelectedCoupon({...selectedCoupon, max_discount_amount: Number(e.target.value)})}
                />
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setSelectedCoupon({...selectedCoupon, valid_from: e.target.value})}
                />
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setSelectedCoupon({...selectedCoupon, valid_until: e.target.value})}
                />
              </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleEditCoupon}>Update Coupon</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
