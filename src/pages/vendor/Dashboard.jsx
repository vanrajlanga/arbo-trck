  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8">
        <h1 className="text-2xl font-bold">Welcome to your Vendor Dashboard</h1>
            ...
            <div className="lg:col-span-2 space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Overview of your upcoming scheduled treks
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <Table>
                    ...
                    <TableBody>
                      {upcomingTreks.map((trek) => (
                        <TableRow key={trek.id}>
                          ...
                          <TableCell>{trek.date}</TableCell>
                          <TableCell className="hidden md:table-cell">
                            <div className="flex items-center space-x-2">
                              <Progress value={calculateProgress(trek.slots.booked, trek.slots.total)} className="h-2" />
                  ...
                  <div className="mt-4 text-center">
                    <Link to="/vendor/treks" className="text-sm text-aorbo-teal hover:underline">
                      View all treks
                    </Link>
                  </div>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="hidden md:table-cell">Trek</TableHead>
                        <TableHead className="text-right">Status</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {recentBookings.slice(0, 3).map((booking) => (
                        <TableRow key={booking.id}>
                          ...
                          <TableCell className="hidden md:table-cell">{booking.trek}</TableCell>
                          <TableCell className="text-right">
                            <span
                  ...
                  <div className="mt-4 text-center">
                    <Link to="/vendor/bookings" className="text-sm text-aorbo-teal hover:underline">
                      View all bookings
                    </Link>
                  </div>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {alerts.map((alert) => (
                      <div key={alert.id} className="flex items-start space-x-4 p-3 rounded-md bg-slate-50">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center 
                          ${alert.type === 'refund' ? 'bg-amber-100 text-amber-600' : 
                            alert.type === 'review' ? 'bg-blue-100 text-blue-600' : 
                            'bg-green-100 text-green-600'}`}>
                          {alert.type === 'refund' ? <Clock className="h-4 w-4" /> : 
                           alert.type === 'review' ? <BarChart className="h-4 w-4" /> : 
                ...
                <CardContent>
                  <div className="space-y-4">
                    {recentReviews.slice(0, 2).map((review) => (
                      <div key={review.id} className="p-3 rounded-md bg-slate-50">
                        <div className="flex justify-between items-start">
                          <h4 className="font-medium text-sm">{review.customerName}</h4>
                          <div className="text-sm">{renderRating(review.rating)}</div>
                        </div>
                        <p className="text-xs text-gray-500 mt-1">{review.trek} • {review.date}</p>
                        <p className="text-sm mt-2">{review.comment}</p>
                      </div>
                    ))}
                    <div className="text-center">
                      <Link to="/vendor/reviews" className="text-sm text-aorbo-teal hover:underline">
                        View all reviews
                      </Link>
                    </div>
                  </div>
                ...
                <TableBody>
                  {recentBookings.map((booking) => (
                    <TableRow key={booking.id}>
                      <TableCell className="font-medium">{booking.id}</TableCell>
                      <TableCell>{booking.customerName}</TableCell>
                      <TableCell>{booking.trek}</TableCell>
                    ...
                    <CardContent className="p-6">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          ...
                          <p className="text-sm text-gray-500">{review.trek} • {review.date}</p>
                        </div>
                        <div className="text-lg">{renderRating(review.rating)}</div>
                      </div>
        ...
        <TabsContent value="alerts">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Recent alerts and notifications that require your attention
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {alerts.map((alert) => (
                  <div key={alert.id} className="flex items-start space-x-4 p-4 rounded-md border">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center 
                      ${alert.type === 'refund' ? 'bg-amber-100 text-amber-600' : 
                        alert.type === 'review' ? 'bg-blue-100 text-blue-600' : 
                        'bg-green-100 text-green-600'}`}>
                      {alert.type === 'refund' ? <Clock className="h-5 w-5" /> : 
                       alert.type === 'review' ? <BarChart className="h-5 w-5" /> : 
