...
const VendorCoupons = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            // Fetch trek coupons
            const { data: trekCouponsData, error: trekCouponsError } =
                await supabase.from("trek_coupons").select("*");

            if (couponsError || trekCouponsError) {
                console.error(
                    "Error fetching data:",
                    couponsError || trekCouponsError
                );
                // Fallback to localStorage
                const savedCoupons = localStorage.getItem("adminCoupons");
                const savedTrekCoupons =
                    localStorage.getItem("vendorTrekCoupons");
                ...
                localStorage.setItem(
                    "adminCoupons",
                    JSON.stringify(formattedCoupons)
                );
                localStorage.setItem(
                    "vendorTrekCoupons",
                    JSON.stringify(trekCouponsData || [])
                );
            }
        ...
        try {
            if (isActive) {
                ...
                if (error) {
                    ...
                    localStorage.setItem(
                        "vendorTrekCoupons",
                        JSON.stringify(updatedTrekCoupons)
                    );
                } else {
                    await fetchData();
                }
                ...
                if (error) {
                    // Fallback to localStorage
                    const updatedTrekCoupons = trekCoupons.filter(
                        (tc) =>
                            !(
                                tc.coupon_id === couponId &&
                                tc.trek_id === trekId
            ...
            toast.success(
                isActive
                    ? "Coupon activated for trek"
                    : "Coupon deactivated for trek"
            );
        } catch (error) {
            ...
            <Card>
                ...
                <CardContent>
                    <div className="space-y-3">
                        {treks.map((trek) => {
                            const activeCoupons = getActiveCouponsForTrek(
                                trek.id
                            );
                            return (
                                <div
