            ...
            <CardContent>
              {filteredWithdrawals.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-gray-500">No {activeTab === 'all' ? '' : activeTab} withdrawal requests found.</p>
                </div>
              ) : (
                    ...
                    <div key={withdrawal.id} className="border rounded-lg p-4">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          <div className="flex items-center gap-2">
                            <h3 className="font-semibold">{withdrawal.id}</h3>
                            <Badge variant="outline" className={getStatusBadgeVariant(withdrawal.status)}>
                              {withdrawal.status.charAt(0).toUpperCase() + withdrawal.status.slice(1)}
                            </Badge>
                          </div>
                          <p className="text-sm text-gray-500">
                            Requested on {new Date(withdrawal.requestDate).toLocaleDateString()} • {withdrawal.selectedBookings.length} bookings
                          </p>
                        </div>
                        <div className="text-right">
                          <div className="text-lg font-semibold">₹{withdrawal.netAmount.toFixed(2)}</div>
                          <div className="text-xs text-gray-500">
                            Total: ₹{withdrawal.amount.toFixed(2)} • Fee: ₹{withdrawal.fees.toFixed(2)}
                          </div>
                        </div>
                      </div>

                      <div className="space-y-2">
                        <div className="text-sm font-medium">Bookings Included:</div>
                        <div className="grid gap-2 mt-1">
                          {withdrawal.selectedBookings.map((booking) => (
                            <div key={booking.id} className="bg-gray-50 p-2 rounded-md flex justify-between items-center text-sm">
                              <div>
                      ...
                      {withdrawal.notes && (
                        <div className="mt-3 pt-3 border-t text-sm">
                          ...
                          <p className="text-gray-600">{withdrawal.notes}</p>
                        </div>
                      )}
                      ...
                      {withdrawal.status === 'completed' && withdrawal.transactionDetails && (
                        <div className="mt-3 pt-3 border-t text-sm">
                          ...
                          <p className="text-gray-600">{withdrawal.transactionDetails}</p>
                          <p className="text-gray-500">Processed on {withdrawal.processedDate ? new Date(withdrawal.processedDate).toLocaleDateString() : 'N/A'}</p>
                        </div>
                      )}
                    </div>
                  ))}
